...
def splat_feat_nd_max(init_grid, feat, coords):
    """
    Args:
        init_grid: B X nF X W X H X D X ..
        feat: B X nF X nPt
    ...
    for ix_d in itertools.product(*l_ix):
        ...
        index = index.long().expand(-1, F, -1)
        src = (feat * wts).to(grid_flat.dtype)
        grid_flat, argmax = scatter_max(src, index, dim=2,out=grid_flat)

    # grid_flat = torch.round(grid_flat)

    return grid_flat.view(init_grid.shape)

def splat_feat_nd_max_confined(init_grid, feat, coords):
    """
    Args:
        init_grid: B X nF X W X H X D X ..
        feat: B X nF X nPt
