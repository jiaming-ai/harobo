...
class PulsarPointsRenderer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        (
            ...
            orthogonal_projection, kwargs, device
        )
        self.focal_length = focal_length.unsqueeze(0)
        # self.sensor_width = sensor_width.unsqueeze(0)
        # self.principal_point_x = principal_point_x.unsqueeze(0)
        # self.principal_point_y = principal_point_y.unsqueeze(0)
        self.znear = znear.item()
        self.zfar = zfar.item()

        self.intrinsics_matrix = torch.cat([
                        focal_length,
                        sensor_width,
                        principal_point_x,
                        principal_point_y,
    ...
    def _extract_intrinsics(  # noqa: C901
        self, orthogonal_projection, kwargs, device, cloud_idx=0
    ...
    def _extract_extrinsics(
        ...
        # Shorthand:
        cameras = self.rasterizer.cameras
        R = kwargs.get("R", cameras.R)
        T = kwargs.get("T", cameras.T)
        tmp_cams = PerspectiveCameras(
            R=R, T=T, device=R.device
        )
        size_tensor = torch.tensor(
            [self.renderer._renderer.height, self.renderer._renderer.width]
        ).unsqueeze(0).repeat(R.shape[0], 1)

        pulsar_cam = _pulsar_from_cameras_projection(tmp_cams, size_tensor)
        cam_pos = pulsar_cam[:, :3]
        cam_rot = pulsar_cam[:, 3:9]
        return cam_pos, cam_rot

    def _get_vert_rad(
        self, vert_pos, cam_pos, orthogonal_projection, focal_length, kwargs, cloud_idx=0
    ) -> torch.Tensor:
        """
        Get point radiuses.

        params:
            vert_pos: [B x Nx3] tensor of vertex positions.
            cam_pos: [Bx3] tensor of camera positions.
        ...
        # TODO: should we use orthogonal projection for rendering the information gain?
        if orthogonal_projection:
            vert_rad = (
                torch.ones(
            ...
            point_dists = torch.norm((vert_pos -
                                     cam_pos.unsqueeze(1).repeat(1, vert_pos.shape[1], 1)),
    ...
    def batch_render(self, vert_pos,vert_feat, **kwargs) -> torch.Tensor:
        """
        This is the batch rendering version of the forward method.
        We assume the same camera (intrinsic) for all point clouds, while 
        allowing multiple extrinsics (cameras) for multiple or the same point cloud.
        
        The dim of vert_pos and vert_feat will be expanded to [B, N, 3] and [B, N, C] respectively, 
        to match the batch size of the camera extrinsics defined by (R, T)
        vert_pos: [N, 3] tensor of point positions
        vert_feat: [N, C] tensor of point features
        
        The following params should be provided in kwargs:
        R: [N, 3, 3]
        T: [N, 3]
        Gamma:
        
        gamma: tensor of shape [B, 1]
        ...
        """
        # Get extrinsics.
        cam_pos, cam_rot = self._extract_extrinsics(kwargs) # [B, 3], [B, 6]
        bs = cam_pos.shape[0]

        vert_pos = vert_pos.unsqueeze(0).expand(bs, -1, -1) # [B, N, 3]
        vert_feat = vert_feat.unsqueeze(0).expand(bs, -1, -1) # [B, N, C]
        ...
        cam_params = torch.cat(
            (
                cam_pos,
                cam_rot,
                self.intrinsics_matrix.repeat(bs, 1),
            ), dim=1
        ) # [B, 13]
        ...
        vert_rad = self._get_vert_rad(
            vert_pos,
            cam_pos,
            self.orthogonal_projection,
            self.focal_length,
            kwargs,
        ) # [B, N]

        # Clean kwargs for passing on.
        gamma = kwargs["gamma"] # float
        if "first_R_then_T" in kwargs.keys():
            raise ValueError("`first_R_then_T` is not supported in this interface.")
        otherargs = {
            argn: argv
        ...
        if "bg_col" not in otherargs:
            bg_col = torch.zeros(
                vert_feat.shape[-1], device=cam_params.device, dtype=torch.float32
            )
            otherargs["bg_col"] = bg_col
        
        # if we don't need differentiable rendering, we can set max_n_hits to 1 to speed up the rendering
        # if "max_n_hits" not in otherargs:
        #     otherargs["max_n_hits"] = 1
